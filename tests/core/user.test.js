...
describe('User ', () => {
    it('can create a new user', 
        async () => {
            ...
            await expect(async () => await user.new_user(user_data).not.toThrow());
        }
    );
    ...
    it('can get a user by username',
        async () => {
            ...
            retreived_user = await user.get_by_username(user_data.username);
            expect(retreived_user.fullname).toEqual(user_data.fullname);
        }
    )
    it('can return null when user is not there',
        async () => {
            const user_data = test_data.basic_user;
    ...
    it('can return null if the user id is not valid',
        async () => {
            const user_data = test_data.basic_user;
    ...
    it('can return null if the user id is not there',
        async () => {
    ...
    it('can not retreive deleted values',
        async () => {
            const user_data = test_data.basic_user;
            await user.new_user(user_data);
    ...
    it('can not create two users with the same username',
    async () => {
        const user_data = test_data.basic_user;
